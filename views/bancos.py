...
def bancos_view(page: ft.Page):
    ...
    agencia_f = ft.TextField(label="Agência", width=120)
    conta_f = ft.TextField(label="Nº Conta", width=150)
    ...
    # (Campos de cartão permanecem iguais)
    nome_cartao_f = ft.TextField(label="Nome do Cartão", width=200)
    tipo_cartao_f = ft.Dropdown(label="Tipo", width=150,
    ...
    def card_banco(b, cor):
        return ft.Container(
            ...
            content=ft.Column([
                ...
                # EXIBIÇÃO DA AGÊNCIA E CONTA NO CARD
    ...
    # (Função card_cartao permanece igual)
    ...
    def carregar_listas():
        try:
            with db_session() as cur:
                # SELECT ATUALIZADO PARA PEGAR AGÊNCIA E CONTA
                cur.execute(
                    "SELECT id, nome_banco, saldo_inicial, data_criacao, agencia, numero_conta FROM bancos WHERE usuario_id=%s ORDER BY nome_banco",
                    (uid,))
                bancos = cur.fetchall()
                cur.execute(
            ...
            carregar_banco_dd();
            page.update()
        exceptException as ex: print(f"Erro listas: {ex}")
    ...
    def preparar_edicao_banco(b):
        ...
        data_inicial_f.value = b['data_criacao'] or datetime.now().strftime("%d/%m/%Y")
        btn_salvar_banco.text = "ATUALIZAR BANCO";
        page.update()

    # (Funções de excluir e cartões permanecem iguais ao seu original)
