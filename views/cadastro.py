...
from database import get_connection, get_cursor, criar_usuario
...
def salvar_perfil_db(usuario_id: int, dados: dict) -> bool:
    try:
        conn = get_connection()
        cur  = get_cursor(conn)
        cur.execute("SELECT id FROM perfil WHERE usuario_id = %s", (usuario_id,))
        if cur.fetchone():
            cur.execute("""
                UPDATE perfil SET
                    nome=%s, cpf=%s, rg=%s, email=%s,
                    data_nasc=%s, telefone=%s, cep=%s,
                    logradouro=%s, numero=%s, complemento=%s,
                    bairro=%s, cidade=%s, estado=%s,
                    empresa=%s, cargo=%s, salario=%s,
                    dia_pagamento=%s, vale=%s, dia_vale=%s
                WHERE usuario_id=%s
            """, (
                dados["nome"], dados["cpf"], dados["rg"], dados["email"],
                dados["data_nasc"], dados["telefone"], dados["cep"],
                dados["logradouro"], dados["numero"], dados["complemento"],
                dados["bairro"], dados["cidade"], dados["estado"],
                dados["empresa"], dados["cargo"], dados["salario"],
                dados["dia_pagamento"], dados["vale"], dados["dia_vale"],
                usuario_id
            ))
        else:
            cur.execute("""
...
def carregar_perfil_db(usuario_id: int) -> dict:
    try:
        conn = get_connection()
        cur  = get_cursor(conn)
        cur.execute("SELECT * FROM perfil WHERE usuario_id = %s", (usuario_id,))
        row  = cur.fetchone()
        conn.close()
...
def cadastro_view(page: ft.Page):
    ...
    login_f  = ft.TextField(label="E-mail (será seu login)", width=350,
                             icon=ft.icons.EMAIL, visible=novo_user,
                             hint_text="exemplo@email.com")
    senha_f  = ft.TextField(label="Senha (mín. 6 caracteres)", width=250,
                             icon=ft.icons.LOCK, password=True,
                             can_reveal_password=True, visible=novo_user)
    senha2_f = ft.TextField(label="Confirmar Senha", width=250,
    ...
    def on_blur_cpf(e):
        ...
        if len(digits) == 0:
            set_field_state(cpf, False); cpf.error_text = "CPF obrigatório"
        elif len(digits) != 11:
            set_field_state(cpf, False); cpf.error_text = f"CPF incompleto ({len(digits)}/11)"
        elif not validar_cpf(digits):
            set_field_state(cpf, False); cpf.error_text = "CPF inválido"
        else:
    ...
    def on_blur_rg(e):
        ...
        if len(digits) < 7:
            set_field_state(rg, False); rg.error_text = "RG inválido (mín. 7 dígitos)"
        else:
            set_field_state(rg, True)
        rg.update()

    def on_blur_data(e):
        digits = re.sub(r'\D', '', data_nasc.value)
        if len(digits) != 8:
            set_field_state(data_nasc, False); data_nasc.error_text = "Use DD/MM/AAAA"
        else:
            data_nasc.value = f"{digits[:2]}/{digits[2:4]}/{digits[4:]}"
            try:
                dt = datetime.strptime(data_nasc.value, "%d/%m/%Y")
                if dt > datetime.now():
                    set_field_state(data_nasc, False); data_nasc.error_text = "Data futura"
                else:
                    set_field_state(data_nasc, True)
            except ValueError:
                set_field_state(data_nasc, False); data_nasc.error_text = "Data inválida"
        data_nasc.update()

    def on_blur_telefone(e):
        digits = re.sub(r'\D', '', telefone.value)
        if len(digits) < 10:
            set_field_state(telefone, False); telefone.error_text = "WhatsApp inválido"
        else:
            telefone.value = f"({digits[:2]}) {digits[2:7]}-{digits[7:]}" if len(digits)==11 else f"({digits[:2]}) {digits[2:6]}-{digits[6:]}"
    ...
    def on_blur_cep(e):
        ...
        if len(digits) != 8:
            set_field_state(cep, False); cep.error_text = "CEP inválido"
        else:
    ...
    def on_blur_estado(e):
        ...
        if len(estado.value) == 2 and estado.value.isalpha():
            ...
            set_field_state(estado, False); estado.error_text = "UF inválida (ex: RS)"
        estado.update()

    def on_blur_nome(e):
        if len(nome.value.strip().split()) < 2:
            set_field_state(nome, False); nome.error_text = "Digite nome e sobrenome"
        else:
            set_field_state(nome, True)
        nome.update()

    def on_blur_login(e):
        if not re.match(r'^[\w\.-]+@[\w\.-]+\.(com|com\.br|net|org)$', login_f.value.strip().lower()):
            set_field_state(login_f, False); login_f.error_text = "E-mail inválido"
        else:
    ...
    def on_blur_senha(e):
        if len(senha_f.value) < 6:
            set_field_state(senha_f, False); senha_f.error_text = "Mínimo 6 caracteres"
        else:
    ...
    def on_blur_senha2(e):
        if senha2_f.value != senha_f.value:
            set_field_state(senha2_f, False); senha2_f.error_text = "Senhas não coincidem"
        else:
    ...
    def on_blur_obrigatorio(field, label):
        def _blur(e):
            if len(field.value.strip()) < 2:
                set_field_state(field, False); field.error_text = f"{label} obrigatório"
            else:
    ...
    def validar_todos() -> list:
        erros = []
        if novo_user:
            on_blur_login(None); on_blur_senha(None); on_blur_senha2(None)
        ...
        on_blur_nome(None); on_blur_cpf(None); on_blur_rg(None)
        on_blur_data(None); on_blur_telefone(None); on_blur_cep(None); on_blur_estado(None)
        on_blur_obrigatorio(endereco, "Endereço")(None)
        on_blur_obrigatorio(bairro,   "Bairro")(None)
        on_blur_obrigatorio(cidade,   "Cidade")(None)
        for field in [nome, cpf, rg, data_nasc, telefone, cep, endereco, bairro, cidade, estado]:
    ...
    def salvar(e):
        ...
        usuario_id = uid
        if novo_user:
            resultado = criar_usuario(nome=nome.value.strip(), login=login_f.value.strip(), senha=senha_f.value)
            if not resultado["ok"]:
    ...
    conteudo = ft.Column(
        ...
        controls=[
            ...
            ft.Divider(),
            ft.Container(
                visible=novo_user, bgcolor="#E8F5E9", border_radius=10, padding=16,
                content=ft.Column([
            ...
            ft.Container(
                ...
                content=ft.Column([
                    ft.Text("👤 1. IDENTIFICAÇÃO E CONTATO", size=15, weight="bold", color="#1565C0"),
            ...
            ft.Container(
                ...
                content=ft.Column([
                    ft.Text("🏠 2. ENDEREÇO RESIDENCIAL", size=15, weight="bold", color="#E65100"),
                    ft.Text("💡 Preencha o endereço manualmente.", size=11, color=ft.colors.GREY_600, italic=True),
                    ft.Row([cep, endereco, numero, complemento], wrap=True, spacing=10),
                    ft.Row([bairro, cidade, estado], wrap=True, spacing=10),
                ], spacing=10)
            ),
            ft.Container(
                bgcolor="#F3E5F5", border_radius=10, padding=16,
                content=ft.Column([
                    ft.Text("💼 3. DADOS PROFISSIONAIS E RENDA", size=15, weight="bold", color="#6A1B9A"),
    ...
    return ft.View(route="/cadastro", controls=controles)