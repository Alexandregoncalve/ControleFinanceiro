import { NextRequest } from "next/server";
import { z } from "zod";
import { exigirSessao } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { apiOk, apiErro, comTratamentoErro } from "@/lib/api-helpers";

const cartaoUpdateSchema = z.object({
  nomeCartao: z.string().min(1),
  tipo: z.enum(["Crédito", "Débito", "Ambos"]),
  limite: z.number(),
  bancoId: z.number(),
  diaVencimento: z.number().nullable().optional(),
});

export const PUT = comTratamentoErro(
  async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    const sessao = await exigirSessao();
    const { id } = await params;
    const body = await req.json();
    const parsed = cartaoUpdateSchema.safeParse(body);

    if (!parsed.success) {
      return apiErro(parsed.error.issues[0].message);
    }

    const resultado = await prisma.cartao.updateMany({
      where: { id: Number(id), usuarioId: sessao.id },
      data: parsed.data,
    });

    if (resultado.count === 0) return apiErro("Cartão não encontrado.", 404);
    return apiOk({ ok: true });
  }
);

export const DELETE = comTratamentoErro(
  async (_req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    const sessao = await exigirSessao();
    const { id } = await params;

    const resultado = await prisma.cartao.deleteMany({
      where: { id: Number(id), usuarioId: sessao.id },
    });

    if (resultado.count === 0) return apiErro("Cartão não encontrado.", 404);
    return apiOk({ ok: true });
  }
);
