...
const vinculoUpdateSchema = z.discriminatedUnion("tipo", [
  z.object({ tipo: z.literal("CLT"), apelido: z.string().min(1), ativo: z.boolean(), detalhes: detalhesCLTSchema }),
  z.object({ tipo: z.literal("AUTONOMO"), apelido: z.string().min(1), ativo: z.boolean(), detalhes: detalhesAutonomoSchema }),
  z.object({ tipo: z.literal("MEI"), apelido: z.string().min(1), ativo: z.boolean(), detalhes: detalhesMEISchema }),
  z.object({ tipo: z.literal("SIMPLES_NACIONAL"), apelido: z.string().min(1), ativo: z.boolean(), detalhes: detalhesSimplesSchema }),
]);
...
async function sincronizarSubcontas(
  ...
  vinculoId: number,
  apelido: string,
  vinculo: z.infer<typeof vinculoUpdateSchema>
) {
...
export const PUT = comTratamentoErro(
  async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const parsed = vinculoUpdateSchema.safeParse(body);
    if (!parsed.success) return apiErro(parsed.error.issues[0].message);

    const existente = await prisma.vinculoRenda.findFirst({ where: { id: vid, usuarioId: sessao.id } });
    if (!existente) return apiErro("Vínculo não encontrado.", 404);

    const dados = parsed.data;

    await prisma.$transaction(async (tx: PrismaTransaction) => {
      await tx.vinculoRenda.update({
        where: { id: vid },
        data: { apelido: dados.apelido.trim(), ativo: dados.ativo, detalhes: dados.detalhes },
      });

      if (dados.ativo) {
        await sincronizarSubcontas(tx, sessao.id, vid, dados.apelido.trim(), dados);
      } else {
        await tx.subconta.updateMany({ where: { vinculoRendaId: vid }, data: { fixa: 0 } });
      }
    });
...
export const DELETE = comTratamentoErro(
  async (_req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    await prisma.$transaction(async (tx: PrismaTransaction) => {
      await tx.subconta.updateMany({ where: { vinculoRendaId: vid }, data: { fixa: 0, vinculoRendaId: null } });
      await tx.vinculoRenda.delete({ where: { id: vid } });
    });
