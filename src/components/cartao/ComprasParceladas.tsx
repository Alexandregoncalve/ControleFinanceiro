    ...
    <div className="flex flex-col gap-3">
      {!filtrarPorConta && (
        <div className="flex gap-2">
          <div className="flex-1 rounded-lg bg-[#EAF3DE] dark:bg-green-900/40 p-2.5 text-center">
            <p className="text-[10px] font-bold text-[#3B6D11] dark:text-green-400">JÁ PAGO</p>
            <p className="text-sm font-bold text-[#3B6D11] dark:text-green-400">{fmt(resumo.totalPago)}</p>
          </div>
          <div className="flex-1 rounded-lg bg-[#FCEBEB] dark:bg-red-900/40 p-2.5 text-center">
          ...
          <div className="flex-1 rounded-lg bg-[#E6F1FB] dark:bg-blue-900/40 p-2.5 text-center">
            <p className="text-[10px] font-bold text-[#0C447C] dark:text-blue-300">TOTAL GERAL</p>
            <p className="text-sm font-bold text-[#0C447C] dark:text-blue-300">{fmt(resumo.totalGeral)}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2">
        {lista.map((p, i) => {
          ...
          return (
            <div key={i} className="rounded-xl border border-[#E2E8F0] dark:border-white/10 bg-white dark:bg-[#1E293B] p-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{p.descBase || "(sem descrição)"}</p>
                  {!filtrarPorConta && <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">{p.conta}</p>}
                </div>
                <span
              ...
              <div className="mt-2 flex items-center gap-2">
                <div className="h-2 flex-1 rounded bg-gray-100 dark:bg-white/10">
                  <div className="h-2 rounded" style={{ width: `${pct}%`, backgroundColor: info.cor }} />
                </div>
                <span className="flex-shrink-0 text-xs font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500">
              ...
              <div className="mt-2 flex items-center justify-between text-xs">
                ...
                {p.proximaData && <span className="font-medium text-[#0C447C] dark:text-blue-300">Próxima: {p.proximaData}</span>}
              </div>
            </div>
          );
        })}
