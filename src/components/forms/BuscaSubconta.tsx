  ...
  return (
    <div className={`relative flex flex-col gap-1 ${className}`}>
      ...
      <input
        ...
        className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-[#1565C0] focus:ring-1 focus:ring-[#1565C0]"
      />
        ...
        <div className="absolute top-full z-10 mt-1 w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
          {filtradas.map((s) => (
            ...
            >
              <ArrowRight
                size={12}
                className={s.categoriaTipo === "Receita" ? "text-[#2E7D32]" : "text-[#C62828]"}
              />
              <span className="flex-1">{s.nome}</span>
              {s.categoriaTipo && (
                <span
                  className={`text-[10px] ${
                    s.categoriaTipo === "Receita" ? "text-[#2E7D32]" : "text-[#C62828]"
                  }`}
                >
                  ({s.categoriaTipo})
                </span>
